    ...
    pixPerMsec(PIXEL_PER_MSECOND)
{
   ...
   /* QGraphicsRectItem* lineCont = new QGraphicsRectItem(1470, 108, 355, 53, this);
    ...
    for (ite = controler->chordList.begin(); ite != controler->chordList.end(); ++ite)
    {
        ...
        tempText->setHtml("<p align='center'>"+ite->name[0]+"<sub>"+ite->name.mid(1)+"</sub>"+"</p>");
    ...
    // partie temporelle
    lastRefresh = 0;
    controler->chordList[currentChord].fullSongItem->setBrush(QColor(1, 174, 242));
...
{
    if(v) {
        controler->chordList[currentChord].fullSongItem->setBrush(QColor(101, 215, 78));
        isCurrentChordValidated = true;
    }
    else
        controler->chordList[currentChord].fullSongItem->setBrush(QColor(175, 22, 27));
}
